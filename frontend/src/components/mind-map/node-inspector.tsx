...
export function NodeInspector({ mapId }: { mapId?: string }) {
  ...
  // Source badge — read the standardised metadata.source shape stamped by
  // the AI generation paths. When present, show a clickable chip so the
  // student can jump back to the material the map came from.
  const source = (node.data.metadata as Record<string, unknown> | null | undefined)?.source as
    | {
        type: 'ai' | 'note' | 'document' | 'course' | 'lms_course' | 'assignment';
        id?: string;
        title?: string;
        model?: string;
        generatedAt?: string;
    ...
    | undefined;
  const sourceHref =
    source?.type === 'note' && source.id
      ? `/notes?id=${source.id}`
      : source?.type === 'course' && source.id
        ? `/courses/${source.id}`
        : source?.type === 'lms_course' && source.id
          ? `/courses/${source.id}`
          : source?.type === 'document' && source.id
            ? `/knowledge`
            : source?.type === 'assignment' && source.id
              ? `/assignments`
              : null;
  const sourceLabel =
    source?.type === 'ai'
      ? 'AI-generated'
      : source
        ? `From ${source.type.replace('_', ' ')}: ${source.title ?? source.id ?? '(untitled)'}`
        : null;
      ...
      <div className="space-y-3">
        ...
        {sourceLabel ? (
          sourceHref ? (
            <Link
              href={sourceHref}
              className="flex items-center justify-between gap-2 rounded-lg border border-teal/30 bg-teal/8 px-2 py-1.5 text-xs text-teal hover:bg-teal/15"
              title={source?.model ? `Generated by ${source.model}` : undefined}
            >
            ...
            <span
              className="flex items-center gap-1.5 rounded-lg border border-teal/30 bg-teal/8 px-2 py-1.5 text-xs text-teal"
              title={source?.model ? `Generated by ${source.model}` : undefined}
            >
              <Sparkles className="h-3 w-3" /> {sourceLabel}
            </span>
